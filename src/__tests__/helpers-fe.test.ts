...
import { createBucketBudgetData, createMonthlyExpense } from "../server/utils/data-factory/testDataFactory";
...
describe('formatMonthlyExpensesToBucketExpenses', () => {
    it('should accumulate amounts for each bucket and update existing budget data', async () => {
        ...
        expect(result).toEqual([
            {  id:'1', category:'test', amount:0, bucketname: 'rent', household: 'test', currentamount: 1500 },
            {  id:'2', category:'test', amount:0, bucketname: 'groceries', household: 'test', currentamount: 200 },
            {  id:'3', category:'test', amount:0, bucketname: 'electric', household: 'test', currentamount: 0 },
        ]);
    });
    ...
    it('should not update buckets that are not in the monthly expenses', async () => {
        ...
        expect(result).toEqual([
            { id:'1', category:'test', amount:0, bucketname: 'rent', household: 'test', currentamount: 0 },
            { id:'2', category:'test', amount:0, bucketname: 'groceries', household: 'test', currentamount: 0 },
            { id:'3', category:'test', amount:0, bucketname: 'internet', household: 'test', currentamount: 100 },
        ]);
    });
...
describe('formatMonthlyExpensesExpenseDate', () => {
    it('should format the expensedate for each expense', async () => {
        const expenses: MonthlyExpense[] = [
            ...
            createMonthlyExpense({ bucketname: 'rent', expensedate:'2025-03-31T21:58:17.539', amount: 1000 }),
            createMonthlyExpense({ id: '2', bucketname: 'groceries', expensedate:'2025-03-31T21:58:17.539', amount: 200 })
        ]
        ...
        expect(result).toEqual([
            { id:'1', person: createdPerson1, vendor:'test', description:'test', expensedate:'03/31/2025', bucketname: 'rent', amount: 1000 },
            { id:'2', person: createdPerson2, vendor:'test', description:'test', expensedate:'03/31/2025', bucketname: 'groceries', amount: 200 },
        ]);
    });
...
describe('calculateSurplus', () => {
    it('should calculate the monthly total amount and current monthly usage', async () => {
        const budgetData: BudgetData[] = [
            createBucketBudgetData({ amount: 1000, bucketname: 'rent', currentamount: 500 }),
            createBucketBudgetData({ id: '2', amount: 200, bucketname: 'groceries', currentamount: 100}),
            createBucketBudgetData({ id: '3', amount: 300, bucketname: 'electric', currentamount: 200}),
        ]
